...
📖 **Dossier d'architecture détaillé :** Consultez [CONCEPTION_TECHNIQUE_ET_ARCHITECTURE.md](CONCEPTION_TECHNIQUE_ET_ARCHITECTURE.md) pour la spécification technique complète du noyau (bootloader, GDT, IDT, mémoire, scheduler, VFS, drivers, GUI et tests).  
🚀 **Feuille de route & Améliorations futures :** Consultez [FEUILLE_DE_ROUTE_ET_AMELIORATIONS.md](FEUILLE_DE_ROUTE_ET_AMELIORATIONS.md) pour l'analyse approfondie de l'existant et les spécifications complètes de toutes les fonctionnalités à développer (mode graphique 1024x768, préemption, souris, FAT32, Ring 3, pile réseau).
